...
$db = new DB($config->db);
DBAInvoker::Request($db, 'Item', DBA_PATH . 'core/ItemDBA.php');
/**
DBAInvoker::Request($db, 'item_attribute', DBA_PATH . 'item_attribute.php');
DBAInvoker::Request($db, 'item_attribute_type', DBA_PATH . 'item_attribute_type.php');
...
for ($i = 0; $i < count($dbas); $i++) {
	DBAInvoker::Request($db, $dbas[$i], DBA_PATH . $dbas[$i] . '.php');
}
...
DBAInvoker::Alter($db, $schema_new, $schema_old);
*/

 DBAInvoker::Remove($db, 'test');
