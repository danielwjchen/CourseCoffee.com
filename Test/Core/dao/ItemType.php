...
class ItemTypeDAOTestCase extends DAOTestCase implements DAOTestTemplate{
	 ...
	 */
	function setUp() {
		$this->db->perform('TRUNCATE TABLE `item_type`');
		$this->defaultParams = array(
			'name' => mt_rand(4, 5)
		);

		$this->db->perform(
			"INSERT INTO `item_type` (`name`) VALUE (:name)",
			$this->defaultParams);

		$this->defaultObject = $this->db->fetch(
			"SELECT * FROM `item_type` WHERE `name` = :name",
			$this->defaultParams);
	}
	...
	function testInstantiation() {
		$type = new ItemTypeDAO($this->db, $this->defaultParams);
