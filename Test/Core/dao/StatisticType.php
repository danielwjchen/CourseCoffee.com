...
class StatisticTypeDAOTestCase extends DAOTestCase implements DAOTestTemplate{
	 ...
	 */
	function setUp() {
		$this->db->perform('TRUNCATE TABLE `statistic_type`');
		$this->defaultParams = array(
			'name' => mt_rand(4, 5)
		);

		$this->db->perform(
			"INSERT INTO `statistic_type` (`name`) VALUE (:name)",
			$this->defaultParams);

		$this->defaultObject = $this->db->fetch(
			"SELECT * FROM `statistic_type` WHERE `name` = :name",
			$this->defaultParams);
	}
	...
	function testInstantiation() {
		$type = new StatisticTypeDAO($this->db, $this->defaultParams);
