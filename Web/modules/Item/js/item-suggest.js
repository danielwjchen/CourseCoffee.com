 ...
 */
window.ItemSuggest = function(regionName) {
	...
	/**
	 * Get list of items from different vendors
	 *
	 * @param sectionId
	 */
	this.getItemList = function(sectionId) {
		region.empty();
		region.addClass('loading');
		region.html('<h3>Please wait while we find the lowest price on your textitems.</h3>' +
		...
		$('.double-underline a', region).click(function(e) {
			...
			var content = '<h2>How does this work?</h2>' +
				"<p>It's a simple idea, really. We capture the most lengendary textitem vendors on the internet (Amazon, Chegg, ValoreItems, eCampus, e.t.c.), and cage them in a gladiator battle royal duel to provide you the best deal possible, pokemon style.<p>" +
			...
			;
			dialog.open('item-search-explained', content);
			...
			success: function(response) {
				...
				if (response.list) {
					...
					$.each(response.list, function(title, value) {
						...
						var itemCover = value['image'] != '' ? '<img src="' + value['image'] + '" class="cover" />' : '';
						html += '<li class="item">' +
							itemCover + 
							'<div class="info">' +
					...
					// share result
					var uri = window.location.protocol + '//' + window.location.host + '/item-search/' + response.uri;
					...
					// debug
					// console.log('item suggest cache');
					// console.log(cache);
	...
	/**
	 * Get all required items for a list of sections
	 */
	this.getAllItemList = function(sectionForm) {
		region.addClass('loading');
		var sectionId = '';
		var html = '';
