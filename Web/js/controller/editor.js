$(document).ready(function(){
						  ...
						  reg[1]=/(0?[1-9]|1[0-2]){1}\/(1[0-9]|2[0-9]|3[0-1]|0?[1-9]){1}(?=[^\/0123456789])/g;	// mm/dd(/2011 2012)
						  ...
						  reg[5]=/(0?[1-9]|1[0-2]){1}-(1[0-9]|2[0-9]|3[0-1]|0?[1-9]){1}(?=[^0123456789])/gi
						  //reg[5]=/week\W+\d{1,2}/gi
						  schedule_list = []
						  //content = ""
						  word = [];
						  delim = [];
									...
									for(m=0;m<reg.length;m++){
										...
										if(hits != null){
											...
											for(i=0;i<hits.length;i++){
												//console.log(hits[i],Date.parse(hits[i]))
											...
											if(variance(inc_val) < min_var && inc_val.length > 10){
						  ...
						  init();
										...
										//alert(start_pos_final.toString() + end_pos_final.toString())
										...
										for(i=end_pos_final; i<schedule_list.length-1; i++){
											for(j=i+1; j<schedule_list.length; j++){
												...
												if( (curr_day-start_day) >= 0 && (curr_day-start_day) < 15){// && j<schedule_list.length-1 ){
													i=j
													console.log("con")
													continue
												}
